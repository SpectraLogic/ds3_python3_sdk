...
def populateTestData(client, bucketName, dataPolicyId, resourceList = None, prefix = "", metadata = None, createBucket=True):
    ...
    fileList = list(map(getSize, resourceList))
...
class Ds3TestCase(unittest.TestCase):
    ...
    def checkBadInputs(self, testFunction, inputs):
        for test_input, status in list(inputs.items()):
            try:
...
class BucketTestCase(Ds3TestCase):
    def testPutBucket(self):
        """tests putBucket"""
        self.createBucket(str(bucketName))

        getService = self.client.get_service(GetServiceRequest())
        self.assertEqual(getService.response.status, 200)
        bucketSet = frozenset([service['Name'] for service in getService.result['BucketList']])
    ...
    def testGetFilledBucket(self):
        ...
        returnedFileList = [(obj['Key'], obj['Size']) for obj in bucketContents.result['ContentsList']]
    ...
    def testGetService(self):
        ...
        servicesBefore = [service['Name'] for service in beforeResponse.result['BucketList']]
        self.assertFalse(bucketName in servicesBefore)
        
        self.createBucket(bucketName)
        
        afterResponse = self.client.get_service(GetServiceRequest())
        servicesAfter = [service['Name'] for service in afterResponse.result['BucketList']]
...
class JobTestCase(Ds3TestCase):
    def testGetJobs(self):
        ...
        fileObjects = FileObjectList([FileObject(obj['Key']) for obj in bucketContents.result['ContentsList']])
        ...
        result = [obj['JobId'] for obj in jobsBefore.result['JobList']]
...
class ObjectTestCase(Ds3TestCase):
    def validateSearchObjects(self, objects, resourceList = resources, objType = "DATA"):
        ...
        if len(objects)>0:
            self.assertEqual(len(set([obj['BucketId'] for obj in objects])), 1)
    ...
    def testGetObject(self):
        ...
        self.assertEqual(fileList[0].size, 294059)
        ...
        self.assertEqual(os.stat(tempname).st_size, 294059)
    ...
    def testGetAndPutObjectStream(self):
        ...
        self.client.put_object(PutObjectRequest(bucketName, fileName, 294059, localFileStream))
        localFileStream.close()
    ...
    def testDeleteObjectUnicode(self):
        ...
        self.client.delete_object(DeleteObjectRequest(bucketName, str("beowulf.txt")))
    ...
    def testDeleteObjects(self):
        ...
        deleteFiles = DeleteObjectList([DeleteObject(obj.name) for obj in fileList])
    ...
    def testDeleteObjectsUnicode(self):
        ...
        deleteList = DeleteObjectList([DeleteObject(obj.name) for obj in fileList])
    ...
    def testGetBulkUnicode(self):
        ...
        objects = FileObjectList([FileObject(str(obj['Key'])) for obj in bucketContents.result['ContentsList']])
...
class ObjectMetadataTestCase(Ds3TestCase):
    ...
    def testGetBulkWithMetadata(self):
        ...
        objects = FileObjectList([FileObject(obj['Key']) for obj in bucketContents.result['ContentsList']])
        bulkGetResult = self.client.get_bulk_job_spectra_s3(GetBulkJobSpectraS3Request(bucketName, objects))
...
class SpecialCharacterTestCase(Ds3TestCase):
    def testObjectNameWithSpace(self):
        ...
        self.client.put_object(PutObjectRequest(bucketName, objectName, 294059, localFile))
        localFile.close()
...
class NotificationsTestCase(Ds3TestCase):
    ...
    def testObjectPersistedRegistration(self):
        ...
        fileList = list(map(getSize, resources))
        fileObjectList = FileObjectList(fileList)
    ...
    def testGetBigFile(self):
        ...
        for key, val in getObjectResult.response.getheaders():
            if key == 'content-length':
                self.assertEqual(int(val), fileSize)
